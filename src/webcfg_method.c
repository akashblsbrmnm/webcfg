...
void handleMethod_rbus(const param_t paramVal[], int methodCount, WDMP_STATUS *retStatus, int *ccspRetStatus)
{
    if(!paramVal || methodCount<=0)
    {
        WebcfgError("invokeMethods_rbus: Invalid input parameters\n"); 
        return;
    }
    ...
    if (!rbus_handle)
    {
        WebcfgError("invokeMethods_rbus: rbus_handle not initialized\n");
        return;
    }
    ...
    for (int i = 0; i < methodCount; i++)
    {
        ...
        {
            WebcfgError("invokeMethods_rbus: Invalid param at index %d\n", i);
        ...
        *retStatus = mapStatus(*ccspRetStatus);
        ...
        {
            WebcfgError("Method %s failed with rbus_error: [%d], ccspStatus: [%d]\n", paramVal[i].name, ret, *ccspRetStatus);
